@section('title', 'Edit Somitee Day | MicroFinance Pro')
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h1 class="h4 mb-1">
                            ...
                            Edit Collection Day - {{ $somiteeDay->weekday }}
                        </h1>
                        <p class="text-muted mb-0">Update collection day information</p>
                    </div>
                </div>
            </div>
            <div class="modern-card-body">
                <form action="{{ route('somitee_days.update', $somiteeDay->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('weekday') is-invalid @enderror" 
                                ...
                                @foreach($weekdays as $key => $value)
                                    <option value="{{ $key }}" 
                                        {{ old('weekday', $somiteeDay->weekday) == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="time" class="form-control @error('collection_time') is-invalid @enderror" 
                                   ...
                                   value="{{ old('collection_time', $somiteeDay->collection_time ? \Carbon\Carbon::parse($somiteeDay->collection_time)->format('H:i') : '') }}">
                        ...
                        <div class="col-12">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="is_active" 
                                       name="is_active" {{ old('is_active', $somiteeDay->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      placeholder="Enter description or notes about this collection day...">{{ old('description', $somiteeDay->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('somitee_days.show', $somiteeDay->id) }}" class="btn btn-outline-info">
                                <i class="fas fa-eye me-2"></i>View Details
                            </a>
                        </div>
                        <button type="submit" class="modern-btn modern-btn-warning">
                            <i class="fas fa-save me-2"></i>Update Collection Day
                        </button>
                    </div>
                </form>
            </div>
