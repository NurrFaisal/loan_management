    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <h1 class="fs-4 mb-3">Create New Loan</h1>
                ...
                <form action="{{ route('loans.store') }}" method="POST" id="loanForm">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="member_id" name="member_id" required>
                            ...
                            @foreach ($members as $member)
                                <option value="{{ $member->id }}" data-somitee="{{ $member->somitee_id }}" {{ old('member_id') == $member->id ? 'selected' : '' }}>{{ $member->name }}</option>
                            @endforeach
                        </select>
                        @error('member_id')
                            <div class="text-danger">{{ $message }}</div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control" id="loan_amount" name="loan_amount" value="{{ old('loan_amount') }}" required>
                        @error('loan_amount')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control" id="interest" name="interest" value="{{ old('interest') }}" required>
                        @error('interest')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control" id="total_payable" name="total_payable" value="{{ old('total_payable') }}" readonly required>
                        @error('total_payable')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="loan_type" name="loan_type" required>
                            ...
                            <option value="Weekly" {{ old('loan_type') == 'Weekly' ? 'selected' : '' }}>Weekly</option>
                            <option value="Monthly" {{ old('loan_type') == 'Monthly' ? 'selected' : '' }}>Monthly</option>
                        </select>
                        @error('loan_type')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control" id="installment" name="installment" value="{{ old('installment') }}" readonly required>
                        @error('installment')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize member filtering if there's an old somitee selection
    if (somiteeSelect.value) {
        somiteeSelect.dispatchEvent(new Event('change'));
    }

    // Initialize calculations if there are old values
